...
describe('Lexer and Tokenizer', () => {
    ...
    test('recognizes operator and number', () => {
        expect(Lexer('%20', Tokenizer)).toEqual([{value:'%', type:'operator'}, {value:'20', type:'number'}])
    })

    test('recognizes number with unit', () => {
        expect(Lexer('20m', Tokenizer)).toEqual([{value:'20', type:'number'}, {value:'m', type:'unit'}])
        expect(Lexer('1h', Tokenizer)).toEqual([{value:'1', type:'number'}, {value:'h', type:'unit'}])
    })
